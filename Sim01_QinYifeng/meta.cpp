...
void meta::readMeta(config& x) {
    ...
    metaFile.open(x.getMetaPath()); /// opens the file that was given in the config file
    ...
    line.erase(remove(line.begin(),line.end(),' '),line.end()); /// removes all the spaces from the string
    line.pop_back(); ///removes the last character because in my files there was a carriage return;
    ...
    if(line != start){ /// checks if the start line matches the correct format and spelling
        ...
        cycles.push_back(0); ///gives dummy varaiable
    ...
    line.pop_back(); ///removes the last character in the string, my files had a carriage return at the end of string
    line.erase(remove(line.begin(),line.end(),' '),line.end()); /// removes all the white space from the string
    ...
    if(!checkStart(temp)) /// calls a function to see if the meta data is correct
    ...
    while(saveData(temp, x)) { ///recurisve call until save data returns false, I pass the temp variable and the class

        if(line.empty()){ /// check it the line is empty
            meta_data.pop_back(); /// my program stored the las meta dat of each line twice so i deleted one
            cycles.pop_back();
            index--;
 ...
 */
bool meta::checkStart(string str) {
 ...
 */
bool meta::saveData(string str , config& x) {
